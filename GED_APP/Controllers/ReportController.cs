...
namespace GED_APP.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public ReportController(IDocument docRepo)
        {
            _docRepo = docRepo;
        }
        ...
        public ActionResult print(string annee)
        {
            try
            {
                ...
                byte[] bytes = prepareReportDocByYear(annee);
        ...
        public byte[] prepareReportDocByYear(string annee)
        {
            ...
            if (annee != null)
            {
                ...
                if (list != null)
                {
                    ...
                    ICollection<Doc> pvs = list.Where(d => d.TypeDoc == "PV").ToList();
                    ICollection<Doc> arretes = list.Where(d => d.TypeDoc == "ARR").ToList();
                    ICollection<Doc> communiques = list.Where(d => d.TypeDoc == "CRP").ToList();
                    ICollection<Doc> autres = list.Where(d => d.TypeDoc == "AUTRE").ToList();
                    if (pvs != null)
                    {
                        // titre PV
